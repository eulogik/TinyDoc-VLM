...
def collate_fn(batch, processor, max_seq_length=256):
    ...
    for img, prompt, answer in zip(images, prompts, answers):
        ...
        # Expand the single <image> placeholder into the correct number
        # of <image> tokens so the tokenizer produces one token per visual
        # feature — the model's forward pass inserts visual embeddings at
        # every position matching image_token_id.
...
def train(
    ...
    nw = num_workers if num_workers is not None else (0 if device == "mps" else 2)
    loader = DataLoader(
